<template>
    ...
    <Head>
        <title>Kategori Naskah − Admin {{ setting.site_title }}</title>
    </Head>

    <div class="app-main flex-column flex-row-fluid" id="kt_app_main">
        <div class="d-flex flex-column flex-column-fluid">
            <div v-if="manuscript_categories.data.length < 2" class="d-flex align-items-center gap-4 bg-myprimary-light p-6 px-lg-8">
                ...
                <div class="d-flex flex-column">
                    ...
                    <span class="text-myprimary fw-semibold fs-7">
                        Masukkan setidaknya 2 kategori naskah agar tampilan di website muncul & terlihat lebih proporsional & sejajar.
                    </span>
                </div>
            </div>
                ...
                <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                        <ol class="breadcrumb text-muted fs-6 fw-semibold mb-3">
                            ...
                            <li class="breadcrumb-item pe-3 text-muted">Kategori Naskah</li>
                        </ol>
                        <div class="page-heading">
                            <h1 class="mb-0 fw-bolder text-dark">Kategori Naskah</h1>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card rounded-4 border border-gray-300">
                        <div class="d-flex flex-grow border-0 p-6 gap-4 flex-column flex-md-row">
                            ...
                            <div class="d-flex gap-4">
                                <Link href="/admin/manuscript-categories/create" type="button"
                        ...
                        <div class="card-body p-0">
                            <div class="text-center border-top border-gray-300 py-10 px-6" v-if="manuscript_categories.data.length === 0">
                                <img src="/assets/media/illustrations/empty.png"
                                    alt="Tidak ada data" class="mh-225px img-fluid mb-10">
                                <h2 class="text-dark mb-3 fw-bolder">Kategori Naskah Tidak Ditemukan</h2>
                                <p class="text-gray-600 fs-4 mb-0">Belum ada kategori naskah atau pencarian tidak sesuai.</p>
                            </div>
                                ...
                                <table
                                    ...
                                    <thead>
                                        <tr class="text-start text-gray-600 fw-bold fs-7 text-uppercase gs-0">
                                            ...
                                            <th class="bg-secondary min-w-325px" style="letter-spacing: 0.04em;">Kategori</th>
                                    ...
                                    <tbody class="fw-semibold text-gray-600">
                                        <tr v-for="(category, index) in manuscript_categories.data" :key="category.id">
                                            <td class="ps-6">
                                                {{ ++index + (manuscript_categories.current_page - 1) * manuscript_categories.per_page }}
                                            </td>
                                            <td class="d-flex align-items-center gap-4">
                                                <div class="ratio ratio-1x1 w-40px w-md-45px flex-shrink-0">
                                                    <img class="h-100 rounded-3 img-fluid object-fit-cover"
                                                        :src="category.image_url || '/assets/media/illustrations/blog-placeholder.png'"
                                                        alt="Gambar Kategori Naskah" />
                                                </div>
                                                <div>
                                            ...
                                            <!-- <td>
                                                <div class="d-flex flex-column gap-1">
                                                    <span class="fs-7">{{ category.requirements.length }} persyaratan</span>
                                                    <span class="fs-7">{{ category.attachments.length }} lampiran</span>
                                                    <span class="fs-7">{{ category.document_count }} dokumen</span>
                                                </div>
                                            </td> -->
                                            <td>
                                                <div class="d-flex flex-column">
                                                    <span class="text-dark fw-bolder fs-5 mb-1">{{ formatDate(category.created_at) }}</span>
                                                    <span class="fs-7">{{ formatTime(category.created_at) }}</span>
                                                </div>
                                            </td>
                                            <td class="text-end pe-6">
                                                <Link :href="`/admin/manuscript-categories/${category.id}/edit`" type="button"
                            ...
                            <Pagination v-if="manuscript_categories.data != 0"
                                :links="manuscript_categories.links" align="end" class="py-5 px-6" />
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        setup() {
            ...
            const handleSearch = () => {
                router.get('/admin/manuscript-categories', {
                    q: search.value,
                });
            };
            ...
            const destroy = (id) => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        router.delete(`/admin/manuscript-categories/${id}`);
                        Swal.fire({
                            icon: 'success',
                            title: 'Dihapus!',
                            text: 'Data Kategori Naskah Berhasil Dihapus!',
                            timer: 2000,
