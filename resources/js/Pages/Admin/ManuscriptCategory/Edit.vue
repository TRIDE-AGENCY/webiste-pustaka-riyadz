<template>
    ...
    <Head>
        <title>Edit Kategori Naskah − Admin {{ setting.site_title }}</title>
    </Head>
                ...
                <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                        ...
                        <div class="page-heading">
                            <h1 class="mb-0 fw-bolder text-dark">Edit Kategori</h1>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <form @submit.prevent="submit">
                            ...
                            <div class="col-md-6 col-xl-7">
                                ...
                                <div class="card rounded-4 border border-gray-300 mt-6">
                                    ...
                                    <div class="card-body p-6">
                                        <div v-for="(item, index) in form.attachments" :key="`att-${index}`" class="d-flex gap-3 mb-4">
                                            <input type="text" required class="form-control rounded-3 fs-5" v-model="form.attachments[index]" placeholder="Masukkan poin lampiran..." />
                                ...
                                <div class="card rounded-4 border border-gray-300 mt-6">
                                    ...
                                    <div class="card-body p-6">
                                        ...
                                        <div v-if="existingDocuments.length" class="mt-5">
                                            <div v-for="(document, index) in existingDocuments" :key="`${document.path}-${index}`"
                                                ...
                                                <div class="d-flex align-items-center gap-4 min-w-0">
                                                    ...
                                                    <div class="min-w-0">
                                                        <a :href="document.url" target="_blank" class="mb-1 d-block text-dark fw-bolder fs-6 text-truncate-1">{{ document.name }}</a>
                                                        <p class="mb-0 fs-7 text-gray-600">{{ formatFileSize(document.size) }}</p>
                                                    </div>
                                                </div>
                                                <button type="button" class="btn btn-sm btn-icon btn-mydanger-tinted rounded-3" @click="removeExistingDocument(index)">
                                                    <i class="ri-delete-bin-fill fs-3"></i>
                                                </button>
                                            </div>
                                        </div>
                        ...
                        <div class="d-flex justify-content-end mt-7">
                            ...
                            <button type="submit" id="submit_button" class="btn btn-sm btn-myprimary-filled rounded-3 fw-bolder fs-5">
                                <span class="indicator-label">Edit Kategori</span>
...
<script>
    ...
    export default {
        ...
        props: {
            setting: Object,
            errors: Object,
            manuscript_category: Object,
        },

        setup(props) {
            const imageItemError = ref('');
            const documentItemError = ref('');
            const existingDocuments = ref([...(props.manuscript_category.existing_documents || [])]);
            const removedDocuments = ref([]);

            const form = reactive({
                title: props.manuscript_category.title,
                description: props.manuscript_category.description,
                image: null,
                requirements: props.manuscript_category.requirements?.length ? [...props.manuscript_category.requirements] : [''],
                attachments: props.manuscript_category.attachments?.length ? [...props.manuscript_category.attachments] : [''],
                attached_documents: [],
                submission_method: props.manuscript_category.submission_method?.length ? [...props.manuscript_category.submission_method] : [''],
            });
            ...
            onMounted(() => {
                new Dropzone('#category-dropzone', {
                    ...
                    init: function () {
                        ...
                        if (props.manuscript_category.image) {
                            const mockFile = {
                                name: props.manuscript_category.image_name ?? 'Gambar Kategori',
                                size: props.manuscript_category.image_size ?? 12345
                            };
                            this.emit('addedfile', mockFile);
                            this.emit('thumbnail', mockFile, `/storage/${props.manuscript_category.image}`);
            ...
            const removeExistingDocument = (index) => {
                ...
                if (removed?.path) {
                    removedDocuments.value.push(removed.path);
                }
            };
            ...
            const submit = () => {
                ...
                removedDocuments.value.forEach((item, index) => payload.append(`removed_documents[${index}]`, item));
                ...
                router.post(`/admin/manuscript-categories/${props.manuscript_category.id}`, payload, {
                    onSuccess: () => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil!',
                            text: 'Data Kategori Naskah Berhasil Diperbarui!',
                            timer: 2000,
