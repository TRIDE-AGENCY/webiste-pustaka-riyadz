<template>
    <Head>
        <title>Tambah Layanan − Admin {{ setting?.site_title || 'Panel' }}</title>
    </Head>
                ...
                <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                        ...
                        <div class="page-heading">
                            <h1 class="mb-0 fw-bolder text-dark">Tambah Layanan</h1>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <form @submit.prevent="submit">
                        <div class="row g-6">
                            <div class="col-md-6 col-xl-5">
                                <div class="card rounded-4 border border-gray-300">
                                    ...
                                    <div class="card-body p-6">
                                        <div class="mb-5 fv-row">
                                            ...
                                            <textarea class="form-control rounded-3 fs-5" rows="2" v-model="form.name" required placeholder="Contoh: Penerbitan Buku" :class="{ 'is-invalid': errors.name }"></textarea>
                                        </div>
                                        <div class="fv-row">
                                ...
                                <div class="card rounded-4 border border-gray-300 mt-6">
                                    <div class="card-header p-6 d-flex flex-column m-0 border-bottom border-gray-300" style="min-height: unset;">
                                        <h3 class="required fw-bolder mb-0">Gambar Layanan</h3>
                                    </div>
                                    <div class="card-body p-6">
                                        <div id="service-dropzone" class="dropzone border-dashed border-myprimary rounded-3 p-6 text-center">
                                            <div class="dz-message needsclick flex-column text-center gap-5 p-5">
                                                ...
                                                <div>
                                                    ...
                                                    <span class="fs-7 fw-semibold text-gray-600">Maks. 10 MB • Rasio 4:3</span>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="card-body p-6">
                                        <div v-if="form.sections.length > 0" class="d-flex flex-column gap-6 mb-6">
                                            <div v-for="(sec, idx) in form.sections" :key="idx" class="d-flex gap-6 align-items-start pb-6 border-bottom border-gray-300">
                                                <div
                                                    class="w-45px h-45px bg-myprimary-light text-myprimary fs-2 fw-bold rounded-pill d-flex flex-center flex-shrink-0">
                                                    {{ idx + 1 }}
                                                </div>
                                                <div class="flex-grow-1">
                                                    <div class="mb-5 fv-row">
                                                        <label class="required form-label fs-6">Judul Konten</label>
                                                        <input type="text" required class="form-control rounded-3 fs-5" v-model="sec.title" placeholder="Contoh: Keunggulan Layanan" />
                                                    </div>
                                                    <div class="mb-5 fv-row">
                                                    ...
                                                    <div class="fv-row">
                                                        <label class="required form-label fs-6">Isi Konten</label>
                        ...
                        <div class="d-flex justify-content-end gap-3 mt-7">
                            ...
                            <button type="submit" id="submit_button" class="btn btn-sm btn-myprimary-filled rounded-3 fw-bolder fs-5">
                                <span class="indicator-label">Tambah Layanan</span>
...
export default {
    ...
    setup(props) {
        ...
        // Dropzone & TinyMCE Init disamakan dengan milik Anda.
        const initDropzone = () => {
            ...
            dropzoneInstance.value = new Dropzone('#service-dropzone', {
                ...
                init: function () {
                    this.on('addedfile', file => form.image = file);
        ...
        // Handlers untuk Arrays
        const addSection = () => {
            form.sections.push({ title: '', type: 'list', content: '' });
        };
        const removeSection = (idx) => form.sections.splice(idx, 1);

        const addPackage = () => {
            form.packages.push({ name: '', price: '', badge_color: 'primary', recommendation: '', features: '' });
        };
        const removePackage = (idx) => form.packages.splice(idx, 1);

        watch(() => form.has_packages, (isActivated) => {
            if (isActivated && form.packages.length === 0) {
                addPackage();
            }
        });
        ...
        const submit = () => {
            ...
            // Pre-process Sections & Packages: Ubah textarea \n menjadi array asli
            const processedSections = form.sections.map(sec => {
                if (sec.type !== 'text') {
                    // Pecah by enter, hapus spasi kosong, filter yg empty
                    return { ...sec, items: sec.content.split('\n').map(i => i.trim()).filter(i => i) };
                }
                return sec;
            });
            ...
            // Kirim sebagai string JSON ke Laravel
            ...
            router.post('/admin/services', payload, {
                onSuccess: () => {
                    Swal.fire({
                        icon: 'success', title: 'Berhasil!', text: 'Layanan Berhasil Disimpan!',
